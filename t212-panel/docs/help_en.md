...
1. **Settings → API Keys** — paste your key and secret from the T212 app (Settings → API (Beta) inside the T212 app). Separate keys for demo and live accounts.
2. **Settings → Watchlist** — search for instruments you're interested in and add them to favorites (max 9 in the Warp Mode grid).
3. The **DEMO/LIVE** toggle in the top bar (next to the T212 connection status) shows and switches which account you're currently working on. **LIVE = real money** — red, glowing, always visible.

...
## ZROBIONE (2026-08-04): "Przekaż botowi" (ręczna adopcja pozycji) ujednolicone we wszystkich 3 silnikach + naprawiony brak natychmiastowego trailing checka

Adam kupił ręcznie Rheinmetall (RHMd_EQ) i poprosił o uzbrojenie trailing stopu na tej pozycji. Sprawdzone: "zarządzaj wszystkim" było już włączone (od incydentu SAP), więc bot sam przejął pozycję automatycznie w tej samej minucie co zakup - nic nie trzeba było robić. Przy tej okazji sprawdzony ręczny mechanizm "Przekaż botowi" (`routes/bot.py::adopt_position`, istniejący od 23.07) - **istniał WYŁĄCZNIE dla Micro-Gridu**, Sygnał i EOD nie miały żadnego sposobu na ręczną adopcję pojedynczej pozycji kupionej z ręki. Znaleziona też ta sama luka co w automatycznej adopcji z tej samej nocy: `adopt_position()` nie wywoływał `_manage_trailing_exit` od razu po utworzeniu `ActiveTrade` - świeżo adoptowana pozycja czekała do 60s na pierwszy tick.

**Adam: "tak, dorób ten fix, ujednolić wszystkie boty pod tym względem"** - zrobione:

1. **Micro-Grid** (`routes/bot.py::adopt_position`) - po utworzeniu `ActiveTrade` od razu wołane `bot_engine._manage_trailing_exit(user_id, client, settings)` (błąd tutaj tylko loguje, nie psuje odpowiedzi - kolejny tick i tak by to zrobił).

2. **Sygnał i EOD dostały CAŁKOWICIE NOWE `adopt_position()`/`release_position()`** (`routes/signal.py`, `routes/eod.py`) - w odróżnieniu od Micro-Gridu (który zostawia stop niearmed do 2 progów zysku), Sygnał/EOD ZAWSZE mają aktywny resting stop od wejścia, więc adopcja **od razu wystawia prawdziwe zlecenie STOP na T212** (nie czeka na żaden tick):
   - Sygnał: liczy ATR z `price_feed.get_mini_chart_ohlc`, `stop_loss_price = avg_price - atr*stop_loss_atr_mult`.
   - EOD: `stop_loss_price = avg_price*(1-stop_loss_pct)`, `take_profit_price` sztywnym % (brak "reference_price sprzed spadku" dla adoptowanej pozycji), `drop_pct_at_entry=0`/`size_multiplier=1` jako formalne defaulty.
   - Oba: kolizja z innym silnikiem sprawdzana przez `market_hours.held_by_other_engine` (ten sam mechanizm co Micro-Grid, zapobiega powtórce buga SUp_EQ).

3. **`routes/scalping.py::_annotate_bot_state`** rozszerzone - sprawdza teraz `ActiveTrade`/`SignalTrade`/`EODTrade` per ticker (nie tylko Micro-Grid), zwraca `managed_by` ("bot"/"signal"/"eod"/None) + `on_signal_list`/`on_eod_list`.
